...
// AutoCollection Demo Modal
export const AutoCollectionDemo: Story = {
	args: {
		size: 'regular',
		space: 'regular',
	},
	render: (args) => {
		const AutoCollectionModalExample = () => {
			const [isModalOpen, setIsModalOpen] = useState(false);
			const [currentSetting, setCurrentSetting] = useState('customer-default');
			const [selectedValue, setSelectedValue] = useState('customer-default');
			const [isLoading, setIsLoading] = useState(false);

			const autoCollectionOptions = [
				{
					value: 'customer-default',
					label: 'Customer Default',
					description: "Use the customer's default auto collection setting"
				},
				{
					value: 'on',
					label: 'On',
					description: 'Enable auto collection for this subscription'
				},
				{
					value: 'off',
					label: 'Off',
					description: 'Disable auto collection for this subscription'
				}
			];
			...
			const handleSave = async () => {
				...
				try {
					...
					setCurrentSetting(selectedValue);
					console.log('Auto collection setting saved:', selectedValue);
					setIsModalOpen(false);
				} catch (error) {
					console.error('Error saving auto collection setting:', error);
				} finally {
					setIsLoading(false);
				}
			};
			...
			const handleCancel = () => {
				setSelectedValue(currentSetting);
				setIsModalOpen(false);
			};
			...
			const getSettingColor = (value: string) => {
				switch (value) {
					case 'on':
				...
				<div className="p-6 max-w-md mx-auto">
					<SCard depth="regular" padding="regular" background="white" rounded>
						<SCard.Header
							...
							description="Manage auto collection preferences"
						/>
						<SCard.Content>
							<div className="space-y-4">
								<div>
									<h4 className="text-sm font-medium text-gray-700 mb-2">
										Current Auto Collection Setting:
									</h4>
									<p className={`text-lg font-semibold ${getSettingColor(currentSetting)}`}>
										{getSettingLabel(currentSetting)}
									</p>
								</div>
					...
					<SModal.Root open={isModalOpen} onOpenChange={setIsModalOpen}>
						<SModal.Content size={args.size} space={args.space}>
							<SModal.Header>
								<SModal.Title>Auto Collection Settings</SModal.Title>
								<SModal.Description>
									Configure the auto collection behavior for this subscription
								</SModal.Description>
							</SModal.Header>
							...
							<SModal.Body>
								<div className="py-4">
									<SRadioGroup.Root
										label="Auto Collection Mode"
										description="Choose how auto collection should be handled"
										value={selectedValue}
		...
		return <AutoCollectionModalExample />;
	},
};
